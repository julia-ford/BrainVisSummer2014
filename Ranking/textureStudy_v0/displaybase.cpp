...
/// Ensures that a given value is within the specified boundaries.
/** If the value exceeds one of the boundaries, the boundary itself is
  * returned instead.
  * @param v the value to check on
...
/** Standard destructor for the DisplayBase class. */
DisplayBase::~DisplayBase(){
...
/// An FA (fractional anisotropy) value is guaranteed to range from 0 to 1.
/// @param fa the initial FA value
int faToInt(float fa){
	// After this, hfa values range from 0 to 1 inclusive.
	float hfa = (fa-0.2f)/0.8f;

	// Force hfa to not be negative.
	if(hfa < 0) { hfa = 0; }
	...
	// After this, hfa values range from 0 to 0.792 inclusive.
	hfa *= 0.8;

	// Multiply by 10, ensuring the value is now between 0 and 7.92 inclusive.
	// Explicit cast added to make the compiler give fewer warnings.
	return (int)(hfa/0.1);
}
...
void DisplayBase::SetupDirectDrawContext(const TrialInfoPtr trialInfoPtr,const TrialDataPtr trialDataPtr) {
	...
	for(int t = 0;t<numFibers;t++){
		...
		for(int s = 0;s<numSegs;s++){
			...
			if(trialInfoPtr->GetMriTask() == FA){
				m_orientationValue[t][s] = (faToInt(m_traces.GetEigen(t,s).GetFA())+1.0f)/8.0f;
			}
			else{
